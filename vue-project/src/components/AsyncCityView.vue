			...
			<div class="mx-8 text-white">
				...
				<div class="flex gap-10 overflow-x-scroll">
					<div v-for="hourData in weatherData.hourly" :key="hourData.dt" class="flex flex-col gap-4 items-center">
						...
						<img class="w-auto h-[50px] object-cover" :src="`http://openweathermap.org/img/wn/${hourData.weather[0].icon}@2px.png`
								" alt="" />
						<p class="text-xl">
...
const getWeatherData = async () => {
	try {
		const weatherData = await axios.get(`https://api.openweathermap.org/data/3.0/onecall/timemachine?lat=${route.query.lat}&lon=${route.query.lon}&appid={186bf4866e4be0085316628434ff3867
}&units=imperial`);
